...
export const HomeScreen: React.FC<{ theme: PaceTheme }> = ({ theme }) => {
  ...
  const dims = [
    ...
    { k: 'move', icon: Icons.Move({ size: 32 }), label: L.dim_move, val: state.exerciseRecorded ? state.exerciseDuration : '—', unit: state.exerciseRecorded ? L.u_minute : '', note: state.exerciseRecorded ? (L[`exercise_type_${state.exerciseType}`] || L.moveNote_walk) : L.notMoved, tone: theme.sage, tap: () => nav.push('exerciseHome') },
